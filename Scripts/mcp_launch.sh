#!/bin/sh
# Launcher for the Açaí MCP server, resolved at plugin start. The plugin lives at
...
#   2. `acai-mcp` on PATH (a released tarball / Homebrew / Scripts/mcp_install.sh — the end-user path).
...
for candidate in \
    "$ROOT/.build/artifacts/acai-mcp" \
    "$ROOT/.build/debug/AcaiMCP" \
    "$ROOT/.build/release/AcaiMCP"; do
    if [ -x "$candidate" ]; then
        exec "$candidate" "$@"
    fi
done
...
if command -v acai-mcp >/dev/null 2>&1; then
    exec acai-mcp "$@"
fi

# 3. Build from source. Stderr only — stdout is the JSON-RPC channel.
echo "acai-mcp: no local or installed binary found; building from source (needs a Swift 6 toolchain)…" >&2
swift build -c release --product AcaiMCP --package-path "$ROOT" >&2
exec "$ROOT/.build/release/AcaiMCP" "$@"
