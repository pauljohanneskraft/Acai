...
# Defaults to build-for-testing only: unlike iOS, there's no simulator for macOS, so an actual
# `xcodebuild test` run drives real windows/mouse/keyboard on this Mac. Pass --run to opt in.
#
# Full xcodebuild output goes to LOG_PATH; stdout stays to a concise summary.
#
...
# A real macOS binary needs at least an ad-hoc signature to launch at all on Apple Silicon —
# CODE_SIGNING_ALLOWED=NO makes AMFI kill it on launch with "Acai.app is damaged and can't be
# opened". Ad-hoc signing needs no certificate/team.
xcodebuild "$ACTION" \
