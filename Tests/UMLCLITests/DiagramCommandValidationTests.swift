...
/// Drives `UMLCommand.Diagram.validate()` through ArgumentParser, asserting the message for every
...
struct DiagramCommandValidationTests {
    ...
    private func expectValidationError(_ arguments: [String], contains expected: String) {
        #expect {
            _ = try UMLCommand.parseAsRoot(["diagram"] + arguments)
        } throws: { error in
    ...
    @Test func validationErrorUsesValidationExitCode() {
        do {
            _ = try UMLCommand.parseAsRoot(["diagram"])
