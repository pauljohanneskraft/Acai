...
struct DiffCommandTests {
    ...
    private func parseDiff(_ arguments: [String]) throws -> UMLCommand.Diff {
        let root = try UMLCommand.parseAsRoot(["diff"] + arguments)
        return try #require(root as? UMLCommand.Diff)
    }
