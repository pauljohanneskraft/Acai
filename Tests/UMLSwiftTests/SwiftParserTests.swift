...
struct SwiftParserTests {
    ...
    @Test func structWithProtocolConformance() {
        let source = """        struct Point: Equatable, Hashable {
            let x: Double
            let y: Double
        }"""
    ...
    @Test func enumWithCases() {
        let source = """        enum Direction: String {
            case north = "N"
            case south = "S"
            case east = "E"
            case west = "W"
    ...
    @Test func protocolDeclaration() {
        let source = """        public protocol Repository {
    ...
    @Test func extensionDeclaration() {
        let source = """        extension String: CustomStringConvertible {
    ...
    @Test func genericClass() {
        let source = """        class Box<T: Equatable> {
            var value: T
    ...
    @Test func inheritance() {
        let source = """        class Dog: Animal, Hashable, CustomStringConvertible {
            var breed: String = ""
        }"""
    ...
    @Test func freestandingFunction() {
        let source = """        func helper(x: Int, y: Int) -> Int {
            return x + y
        }"""
    ...
    @Test func typeAlias() {
        let source = """        public typealias StringMap = Dictionary<String, String>"""
    ...
    @Test func multipleGenericConstraints() {
        let source = """        func process<T: Codable & Hashable>(item: T) -> String {
    ...
    @Test func whereClauseGeneric() {
        let source = """        func combine<S: Sequence>(items: S) -> [S.Element] where S.Element: Comparable {
            return items.sorted()
        }"""
    ...
    @Test func attributesAndAnnotations() {
        let source = """        struct User: Codable {
    ...
    @Test func compositionTypes() {
        let source = """        struct Model: Codable & Hashable & Identifiable {
            var id: String
        }"""
    ...
    @Test func variadicParameters() {
        let source = """        func combine(values: Int...) -> Int {
    ...
    @Test func inoutParameters() {
        let source = """        func swap(_ a: inout Int, _ b: inout Int) {
            let temp = a
            a = b
            b = temp
        }"""
    ...
    @Test func finalClass() {
        let source = """        final class Singleton {
