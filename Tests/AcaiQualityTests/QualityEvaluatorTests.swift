...
struct QualityEvaluatorTests {
    ...
    @Test func forbiddenRuleRespectsEdgeKindFilter() {
        ...
        #expect(QualityEvaluator(rules: rules).evaluate(art).isPassing)
    }
    ...
    @Test func moduleCycleIsDetected() {
        ...
        let rules = QualityRules(cycles: CycleRule(scope: .modules))
        let report = QualityEvaluator(rules: rules).evaluate(art)
    ...
    private func breaches(_ metric: MetricBudget.Metric, max: Double, on subject: String,
        ...
        let rules = QualityRules(budgets: [MetricBudget(metric: metric, max: max)])
        return QualityEvaluator(rules: rules).evaluate(art).violations
