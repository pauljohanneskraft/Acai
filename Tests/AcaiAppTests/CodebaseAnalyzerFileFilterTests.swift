...
struct CodebaseAnalyzerFileFilterTests {
    ...
    func blockedFileIsExcludedFromIndexing() async throws {
        ...
        let artifact = try await CodebaseAnalyzer().enrichedArtifact(at: dir, fileFilter: filter)
    ...
    func nilFilterIndexesEverything() async throws {
        let dir = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: dir) }
        try "class Foo {}\n".write(to: dir.appendingPathComponent("Foo.swift"), atomically: true, encoding: .utf8)

        let artifact = try await CodebaseAnalyzer().enrichedArtifact(at: dir, fileFilter: nil)
