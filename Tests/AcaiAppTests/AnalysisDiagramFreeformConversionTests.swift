...
/// Module Coupling and Hotspot are read-only analysis views with no sensible "freeform node"
/// equivalent — see `GeneratedDiagram+Freeform.swift`'s doc comment. `convertToFreeform` handles
/// both explicitly rather than falling through to the `ClassFreeformConversion` default (which
/// would silently misinterpret their artifact as a class diagram): this locks in that each
/// produces an *empty*, named `FreeformDiagram`, not a class-diagram reinterpretation of whatever
/// artifact happens to be passed.
@Suite("Analysis Diagram (Module Coupling / Hotspot) → Freeform Conversion")
@MainActor
struct AnalysisDiagramFreeformConversionTests {

    /// A minimal, non-empty artifact — if either case fell through to `ClassFreeformConversion`,
    /// this would produce a non-empty node list and fail the assertions below.
