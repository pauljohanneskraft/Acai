...
/// `uml_image` — renders a diagram to a PNG (base64 image content the agent can actually see), for
/// every diagram kind. macOS-only: rendering uses SwiftUI's `ImageRenderer`. Mirrors `uml image`.
/// Dispatches to the shared `UMLRender` image exporters.
