    ...
    struct Quality: ParsableCommand {
        static let configuration = CommandConfiguration(
            ...
            curated smell budgets (long parameter lists, data classes, low cohesion, feature envy, …).
              ...
              uml quality --source ./ --rules quality.yml
              uml quality --source ./ --explore            # rank smells + list cycles, never fail
              uml quality --source ./ --rules quality.yml --baseline last-release
            """
        )
...
/// JSON envelope for `uml quality`: the quality verdict plus optional baseline drift. `drift` is
