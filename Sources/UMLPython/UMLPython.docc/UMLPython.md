...
The Python plugin — parses `.py` source into the [UMLCore](/documentation/umlcore/) model with
Tree-sitter.

## Overview

`UMLPython` is a self-contained language plugin built on Tree-sitter (shared helpers come from
[UMLTreeSitter](/documentation/umltreesitter/)). The ``PythonCodeParser`` reports
`SourceLanguage.python` and carries Python's
[LanguageConfiguration](/documentation/umlcore/languageconfiguration); the ``PythonDetector``
