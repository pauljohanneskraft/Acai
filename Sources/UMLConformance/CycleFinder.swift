...
/// conformance rules see. Module adjacency is provenance-aware (via `ModuleAttribution`), so a
/// cross-module extension does not manufacture a phantom upward edge / false cycle.
///
/// Shared by `ConformanceEvaluator` (the `cycle` rule) and the standalone `uml cycles` command so the
/// two never disagree.
