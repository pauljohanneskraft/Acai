...
public struct GitCheckout {
    ...
    /// Incremental fetch of the `origin` remote, not a full re-download; aborts cooperatively if
    /// the calling `Task` is cancelled — see `GitFetch`.
    ...
    /// exposes no merge-base primitive, so this opens its own raw libgit2 handle and calls
    /// `git_merge_base` directly, following `GitWorktree`'s precedent for bypassing
    /// `SwiftGitX.Repository` (and pairing `SwiftGitXRuntime.initialize()`/`.shutdown()` itself,
    /// since that pair otherwise only happens as a side effect of a live `SwiftGitX.Repository`).
    public func mergeBase(_ a: String, _ b: String) throws -> String {
        ...
        // `origin/`-prefixed name, so a branch that only exists as a remote-tracking ref (e.g. a
        // linked worktree's non-default branch) wouldn't resolve by its plain name alone. Try the
        // remote-tracking form first, matching `GitReference.resolveBase`'s priority, then fall
        // back to `revision` as given.
    ...
    /// one checkout at a time (libgit2 rejects a second attempt with "already checked out"), which
    /// a `GitWorktree`'s linked worktrees would violate. Worktree callers must use
    /// `switchToDetached(ref:)` instead.
    ...
    /// branch ref, so it never conflicts with that branch being checked out elsewhere — the
    /// property a `GitWorktree` checkout needs.
