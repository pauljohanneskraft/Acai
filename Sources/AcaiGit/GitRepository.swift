...
public struct GitRepository: Sendable {
    ...
    /// Callers own choosing this location (an app-container path, a test's scratch directory, …);
    /// `GitRepository` only owns the naming scheme beneath it.
    ...
    /// libgit2 (like plain `git`) writes `.git/FETCH_HEAD` on every fetch (and the initial clone),
    /// so its modification date is a reliable "last fetched" signal without this needing any
    /// separate persisted bookkeeping.
    ...
    /// Read-only: never touches the shared clone's own working directory, HEAD, or index — see
    /// `GitDiffSnapshot`.
