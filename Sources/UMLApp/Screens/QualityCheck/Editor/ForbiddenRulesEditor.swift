...
struct ForbiddenRulesEditor: View {
    ...
    private func add() {
        rules.append(DependencyRule(from: UMLQuality.Selector(), to: UMLQuality.Selector()))
    }
...
struct ContractsEditor: View {
    ...
    private func add() {
        contracts.append(StereotypeContract(into: UMLQuality.Selector(), only: UMLQuality.Selector()))
    }
