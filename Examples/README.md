...
Each `Exports/` holds one `<language>.dot` and one `<language>.png` per language. (There's no
...
```sh
# Class diagram — DOT + PNG per language (swift kotlin java typescript dart), macOS only for images
...
The PNGs are stored in **Git LFS** (see [`.gitattributes`](../.gitattributes)); the `.dot`
files are plain text so they stay reviewable in diffs. If you change the samples or the
generators, regenerate the affected exports and re-run `swift test` — the regression tests
compare freshly-generated DOT byte-for-byte and re-render each PNG to confirm it still matches.
