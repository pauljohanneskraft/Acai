...
acai image   --source Examples/ClassDiagram --language <lang> --grouping none \
...
acai image   --source Examples/SequenceDiagram --language <lang> \
    --sequence-from "Checkout.placeOrder" \
...
acai image   --source Examples/StateDiagram --language <lang> \
    --state-from "Download.state" \
...
acai image   --source Examples/CallGraph/<Lang> --language <lang> --call-graph \
    --output Examples/CallGraph/Exports/<lang>.png --scale 2

# Delta diagrams — `acai diff` between two revisions, colour-coding added/removed/changed elements.
...
acai diff --source-old Examples/ClassDiagramDiff/<Lang>/Before \
...
acai diff --source-old Examples/SequenceDiagramDiff/<Lang>/Before \
...
acai diff --source-old Examples/StateDiagramDiff/<Lang>/Before \
...
acai diff --source-old Examples/PackageDiagramDiff/<Lang>/Before \
...
acai diff --source-old Examples/CallGraphDiff/<Lang>/Before \
...
# Delta PNGs (macOS only) — `acai image` with --source-old renders the union with the same colour
# coding, natively (matching the other example PNGs). Add the matching diagram-type flag per type.
acai image --source-old Examples/ClassDiagramDiff/<Lang>/Before \
...
acai image --source-old Examples/SequenceDiagramDiff/<Lang>/Before \
...
acai image --source-old Examples/StateDiagramDiff/<Lang>/Before \
...
acai image --source-old Examples/PackageDiagramDiff/<Lang>/Before \
...
acai image --source-old Examples/CallGraphDiff/<Lang>/Before \
...
> `acai image` is macOS-only — it renders with SwiftUI's `ImageRenderer`, which needs a window-server session. On Linux, use `acai diagram … | dot -Tpng` to produce images via Graphviz instead.
