...
Complete reference for the `acai` command-line tool. For an overview of the project, see the [root README](../README.md); for the agent-facing server, see [MCP.md](MCP.md).

Everything here comes from the binary's own `--help`. Run `acai <command> --help` any time to check against your build.
...
This repository gates itself with its own [`quality.yml`](../quality.yml).
