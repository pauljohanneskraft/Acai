...
final class NewCodebaseSheetScreen {
    ...
    /// literal label across every plausible element kind — acceptable here since the fixture's
    /// repository/ref names are fixed and known ahead of time. Returns the matched option element
    /// (already waited-for) so the caller can assert its existence before tapping.
    @discardableResult
    func choose(_ label: String, from picker: XCUIElement, timeout: TimeInterval = 5) -> XCUIElement {
        picker.tap()
