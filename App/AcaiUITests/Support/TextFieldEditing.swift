...
extension XCUIElement {
    ...
    /// `waitForExistence` already returned true. Polls with a real `Thread.sleep` between reads
    /// rather than `XCTNSPredicateExpectation`: `XCUIElement` isn't KVO-compliant, so a predicate
    /// expectation on `isHittable` latches its first evaluation instead of re-querying (confirmed
    /// empirically too).
